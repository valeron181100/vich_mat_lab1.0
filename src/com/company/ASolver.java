...
public class ASolver {
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        if(tryDiagonallyDominant(matrix)){
            ...
            double[] prevValsTMP = new double[size];
            //Задаем начальное приближение
            for (int i = 0; i < size; i++) {
                prevVals[i] = freeMemberVector[i];
                prevValsTMP[i] = freeMemberVector[i];
            }
            for (;;) {
                ...
                for (int i = 0; i < matrix.length; i++) {
                    ...
                    for (int j = 0; j < matrix.length; j++) {
                        sum += matrix[i][j] * prevValsTMP[j];
                    }
                    curVals[i] = round(sum + freeMemberVector[i], 5);
                    prevValsTMP[i] = curVals[i];
                }
    ...
    static boolean tryDiagonallyDominant(double[][] matrix){
        ...
        ASolver.freeMemberVector = nFreeVector.clone();
        ASolver.matrix = nMatrix.clone();
    ...
    static double[][] transformMatrix(double[][] matrix){
        for (int i = 0; i < matrix.length; i++) {
            ...
            double b = ASolver.freeMemberVector[i];
            ASolver.freeMemberVector[i] = b / diagElem;
            for (int j = 0; j < matrix.length; j++) {
                if(i == j)
                    matrix[i][j] = 0;
                else
